...
class DBM(nn.Module):
    def __init__(self, size, nc, nh, discrete_indices=None, sigma=0.15, L=2):
        super().__init__()
        ...
        self.nc = nc
        self.L = L
        self.register_buffer("sigma", torch.ones(nv) * sigma)
    ...
    def forward(self, v):
        ...
        # Positive phase
        v = v.flatten(1).float()
        print(v.min(), v.max(), v.mean(), v.std(dim=0).median())  # --- DEBUG ---
    ...
    def energy(self, v, h):
        energy = torch.sum(((v.detach().clone() - self.bias[0].unsqueeze(0))/self.sigma.unsqueeze(0))**2/2., 1)

        for i in range(self.L):
            logits = F.linear(v.detach().clone()/self.sigma.unsqueeze(0)**2 if i==0 else h[i-1].detach().clone(),
    ...
    def gibbs_step(self, v, h, fix_v=False,
        ...
        if even.sum() > 0:
            if not fix_v:
                ...
                if T == 0:
                    v_[even] = deepcopy(logits)
                else:
                    logits /= T

                    if rand_v is None:
                        v_[even] = Independent(Normal(loc=logits, scale=self.sigma.unsqueeze(0)), 1).sample()
                    else:
            ...
            for i in range(0, len(h), 2):
                logits = F.linear(v_[even]/self.sigma.unsqueeze(0)**2 if i==0 else h_[i-1][even],
                                  self.weight[i], self.bias[i+1])
                if i+1 < len(h):
                    logits += F.linear(h_[i+1][even],
                                       self.weight[i+1].t(), None)
        ...
        if odd.sum() > 0:
            for i in range(0, len(h), 2):
                logits = F.linear(v_[odd]/self.sigma.unsqueeze(0)**2 if i==0 else h_[i-1][odd],
                                  self.weight[i], self.bias[i+1])
                if i+1 < len(h):
            ...
            if not fix_v:
                ...
                if T == 0:
                    v_[odd] = deepcopy(logits)
                else:
                    logits /= T

                    if rand_v is None:
                        v_[odd] = Independent(Normal(loc=logits, scale=self.sigma.unsqueeze(0)), 1).sample()
                    else:
                        raise NotImplementedError("rand_v used for continuous variables in gibbs_step --- DEBUG ---")
