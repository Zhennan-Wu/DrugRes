...
class DBM(nn.Module):
    def __init__(self, size, nc, nh, ncv, sigma=0.2, L=2):
        super().__init__()
        if (ncv == 0):
            raise ValueError("Pure discrete input should use Bernoulli DBM")
        ...
        self.register_buffer("sigma", torch.ones(ncv) * sigma)
    ...
    def coupling(self, v, h, fix_v=False):
        ...
        while not converged.all():
            ...
            v_, h_ = self.mh_step(_v, _h, fix_v, rand_h, rand_u)
    ...
    def energy(self, v, h):
        energy = torch.sum(((v.detach().clone()[:, :self.ncv] - self.bias[0].unsqueeze(0)[:, :self.ncv]) / self.sigma.unsqueeze(0))**2 / 2., 1) - torch.sum(v.detach().clone()[:, self.ncv:] * self.bias[0].unsqueeze(0)[:, self.ncv:], 1)

        pseudo_sigma = torch.ones(self.nv, device=v.device)
        pseudo_sigma[:self.ncv] = self.sigma
        for i in range(self.L):
            logits = F.linear(v.detach().clone()/pseudo_sigma.unsqueeze(0)**2 if i==0 else h[i-1].detach().clone(),
                              self.weight[i], self.bias[i+1])

            energy = energy - torch.sum(h[i].detach().clone() * logits, 1)
    ...
    def gibbs_step(self, v, h, fix_v=False,
        ...
        if even.sum() > 0:
            if not fix_v:
                ...
                if T == 0:
                    v_[even][:self.ncv] = deepcopy(logits[:, :self.ncv])
                    v_[even][self.ncv:] += (logits[:, self.ncv:] >= 0).float()
                else:
                    logits /= T

                    if rand_v is None:
                        v_[even][:self.ncv] = Independent(Normal(loc=logits[:, :self.ncv], scale=self.sigma.unsqueeze(0)), 1).sample()
                        v_[even][self.ncv:] = Independent(Bernoulli(logits=logits[:, self.ncv:]), 1).sample()
                    else:
                        raise NotImplementedError("rand_v used for continuous variables in gibbs_step --- DEBUG ---")
            ...
            for i in range(0, len(h), 2):
                logits = F.linear(v_[even]/pseudo_sigma.unsqueeze(0)**2 if i==0 else h_[i-1][even],
                                  self.weight[i], self.bias[i+1])
                if i+1 < len(h):
                    logits += F.linear(h_[i+1][even],
                                       self.weight[i+1].t(), None)
        ...
        if odd.sum() > 0:
            ...
            pseudo_sigma[:self.ncv] = self.sigma
            for i in range(0, len(h), 2):
                logits = F.linear(v_[odd]/pseudo_sigma.unsqueeze(0)**2 if i==0 else h_[i-1][odd],
                                  self.weight[i], self.bias[i+1])
                if i+1 < len(h):
            ...
            if not fix_v:
                ...
                if T == 0:
                    v_[odd][:self.ncv] = deepcopy(logits[:, :self.ncv])
                    v_[odd][self.ncv:] += (logits[:, self.ncv:] >= 0).float()
                else:
                    logits /= T

                    if rand_v is None:
                        v_[odd][:self.ncv] = Independent(Normal(loc=logits[:, :self.ncv], scale=self.sigma.unsqueeze(0)), 1).sample()
                        v_[odd][self.ncv:] = Independent(Bernoulli(logits=logits[:, self.ncv:]), 1).sample()
                    else:
                        raise NotImplementedError("rand_v used for continuous variables in gibbs_step --- DEBUG ---")
