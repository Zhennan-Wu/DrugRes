...
def train(rank, args, model, optimizer, scheduler,
          dataloader, writer, epoch, detect_anomaly=True):
    model.train()
    energies_per_batch = []
    loss_per_batch = []
    with torch.autograd.set_detect_anomaly(detect_anomaly):
        for step, (x, y) in enumerate(dataloader):
            ...
            # print(f"Rank {rank}, Epoch {epoch}, Step {step}: training batch")

            optimizer.zero_grad()
            loss = model(v, r)
            # print(f"Rank {rank} Epoch {epoch}, Step {step}: after computing loss")
            # print(f"Rank {rank}: before backward")
            loss.mean().backward()
            # print(f"Rank {rank} Epoch {epoch}, Step {step}: after backward")
            # print(f"Rank {rank}: before optimizer.step()")
            optimizer.step()
            # print(f"Rank {rank} Epoch {epoch}, Step {step}: after optimizer.step()")
            # print(f"Rank {rank}: before scheduler.step()")
            scheduler.step()
            # print(f"Rank {rank} Epoch {epoch}, Step {step}: after scheduler.step()")
...
def run(rank, args):
    ...
    # Model
    model = DBM(args.nv, args.nh, args.ny, args.L, args.y_sigma, args.rho).to(rank)
    ...
    if args.log_dir is None:
        ...
        log_dir = f"./runs/supervised-{args.dataset}-L:{args.L}-ySigma:{args.y_sigma}-rho:{args.rho}-nh:{args.nh}-lr:{args.lr}-momentum:{args.momentum}-bs:{args.batch_size}-gamma:{args.gamma}-epoch:{args.epoch}-seed:{args.seed}-{run_id}"
    else:
...
def load_model(log_dir, device, *args):
    model = DBM(args.nv, args.nh, args.ny, args.L, args.y_sigma, args.rho).to(device)
